...
impl AuthPacket {
    pub fn new(
        ...
        buffer.data_mut().resize(PKT_AUTH_NO_USERNAME_SIZE + username_len as usize, 0);
    ...
    fn new_from_encryted(buffer_handle: BufferHandle) -> Self {
        //let len = buffer_handle.data().get(PKT_AUTH_USERNAME_LEN_RANGE).copied().unwrap();
        Self {
            buffer_handle
        }
    }
    ...
    pub fn is_valid_buffer_size(buffer: &BufferHandle, buffer_size: usize) -> bool {
        if buffer_size > PKT_AUTH_NO_USERNAME_SIZE {
            let username_len = buffer.data()[PKT_AUTH_USERNAME_LEN_RANGE];
            return username_len as usize + PKT_AUTH_NO_USERNAME_SIZE == buffer_size;
        }
        false
    }
